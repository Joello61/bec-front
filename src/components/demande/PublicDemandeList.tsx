...
interface PublicDemandeListProps {
  demandes: PublicDemande[];
  pagination?: PaginationMeta | null;
  onPageChange?: (page: number) => void;
  isLoading?: boolean;
}

export default function PublicDemandeList({ 
  demandes, 
  pagination, 
  onPageChange,
  isLoading = false 
}: PublicDemandeListProps) {
  // ==================== LOADING STATE ====================
  if (isLoading) {
    return (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 gap-4 md:gap-6">
        {demandes.map((demande) => (
          <PublicDemandeCard key={demande.id} demande={demande} />
        ))}
      </div>
