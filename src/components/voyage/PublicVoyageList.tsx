...
import type { PaginationMeta, PublicVoyage } from '@/types';
import { Pagination } from '../common';
import PublicVoyageCard from './PublicVoyageCard';

interface PublicVoyageListProps {
...
export default function PublicVoyageList({ 
  voyages, 
  pagination, 
  onPageChange,
  isLoading = false 
}: PublicVoyageListProps) {
  // ==================== LOADING STATE ====================
  if (isLoading) {
    return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 gap-4 md:gap-6">
        {voyages.map((voyage) => (
          <PublicVoyageCard key={voyage.id} voyage={voyage} />
        ))}
      </div>
